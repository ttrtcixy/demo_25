...
var getPartners = `select PartnerId, PartnerType, PartnerName, Director, Phone, Rating From Partners;`
...
func (d *DB) GetPartners() (*models.Partners, error) {
	...
	for {
		...
		err := rows.Scan(&partner.Id, &partner.PartnerType, &partner.CompanyName, &partner.Director, &partner.Phone, &partner.Rating)
		if err != nil {
			return nil, err
		}
...
var addPartner = `insert into Partners(PartnerType, PartnerName, Director, Phone, Rating) values(?, ?, ?, ?, ?)`

func (d *DB) AddPartner(partner models.Partner) error {
	args := []any{partner.PartnerType, partner.CompanyName, partner.Director, partner.Phone, partner.Rating}
